...
public class ApplicationCommands {
    ...
    public String updateBook(@ShellOption("-id") long bookId,
        ...
        Author author;
        try {
            author = libraryService.getAuthorById(authorId).orElseThrow(() -> new RuntimeException("author does not exist"));
        } catch (EmptyResultDataAccessException e) {
            return "author does not exist";
        }
        Genre genre;
        try {
            genre = libraryService.getGenreById(genreId).orElseThrow(() -> new RuntimeException("genre does not exist"));;
        } catch (EmptyResultDataAccessException e) {
